...
{
	static const unsigned int Receive_Delay_1 = 5040;
	...
	if(*RFM_Command == NEW_RFM_COMMAND){
		unsigned long timerJanela1 = millis();
		#if (SAMR34)
	...
	if(Message_Status == CRC_OK)
	{
		...
		if(Message->MAC_Header == 0x40 || Message->MAC_Header == 0x60 || Message->MAC_Header == 0x80 || Message->MAC_Header == 0xA0)
		{
			...
			//Check MIC
		  	...
		  	{
				Message_Status = WRONG_MESSAGE;
				debug_str("WRONG_MESSAGE: ADDRESS");
		  	}
			...
			if(Message_Status == MIC_OK)
			{
				...
				*Session_Data->Frame_Counter = 0x0000;
				...
				for(uint8_t i = 0; i < 16 ;++i)
					debug_hex(Session_Data->NwkSKey[i]);
				debug_str("\r\nAppSKey: ");
				for(uint8_t i = 0; i < 16 ;++i)
					debug_hex(Session_Data->AppSKey[i]);
				debug_str("\r\m: ");
#endif
				joinStatus = true;
			}
